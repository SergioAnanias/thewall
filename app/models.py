...
class UserManager(models.Manager):
    def validator(self, postData):
        errors={}
        if len(postData["first_names"]) < 3:
            errors["first_names"]="Debe contener al menos 3 caracteres"
        if len(postData["last_names"]) < 3:
            errors["last_names"]="Debe contener al menos 3 caracteres"
...
#Valida si la fecha no esta nula, es una fecha valida, y ademas si es que la fecha esta en pasado
        if len(postData["dateborn"]) == 0 or len(postData["dateborn"]) < 10 or datetime.strptime(postData["dateborn"], "%Y-%m-%d") > datetime.now(): 
            ...
            minedad = 13 * 365 #Minimo de edad para registrarse
            if edad.days < minedad: # Revisa si el usuario tiene cierta edad 
                errors["minage"] = "Debe tener al menos 13 años"
        return errors
# Valida la información del login
    def loginvalidator(self, postData):
        ...
        elif not bcrypt.checkpw(postData['password'].encode(), pw1.encode()):
